...
export function useChatSocket(token: string | null) {
  ...
  // Tracks the last targetCustomerId a message was actually sent with, so
  // we can drop a divider into the transcript when it changes — the whole
  // point being that tool scope follows this field.
  ...
  const sendMessage = useCallback((content: string, targetCustomerId: string) => {
    const ws = wsRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) return;

    setItems((prev) => {
      const next = [...prev];
      if (lastTargetRef.current !== null && lastTargetRef.current !== targetCustomerId) {
        next.push({ id: newId(), kind: 'divider', text: `— now assisting ${targetCustomerId} —` });
      }
      next.push({ id: newId(), kind: 'user', text: content });
      return next;
    });
    lastTargetRef.current = targetCustomerId;
    setIsWaiting(true);
